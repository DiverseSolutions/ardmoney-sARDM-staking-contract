...
describe("Testing Penalty", function () {
  ...
  it("setDeadline Check", async function () {
    const base = await loadFixture(testScenarioSetup);
    const { staking } = base;

    await expect(staking.setPenaltyFee(parse18(11))).to.be.revertedWith("PENALTY FEE ABOVE 10% CAP");
  });

  it("userDeadlineOf : ADDRESS ZERO Revert", async function () {
    const base = await loadFixture(testScenarioSetup);
    const { staking } = base;

    await expect(staking.userDeadlineOf(ethers.ZeroAddress)).to.be.revertedWith("ADDRESS ZERO");
  });
  ...
  it("Penalty", async function () {
    ...
    expect(await staking.hasUserDeadlinePassed(accountA.address)).to.equal(
      false,
    );
    expect(await staking.getTotalLockedARDM()).to.equal(parse18(0));
    expect(await staking.getTotalsARDM()).to.equal(parse18(0));
